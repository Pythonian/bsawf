...
def users():
    ...
    while True:
        ...
        created_on = fake.date_time_between(
            start_date='-1y', end_date='now').strftime('%Y-%m-%dT%H:%M:%SZ')
        ...
        params = {
            ...
            'sign_in_count': random.randint(1, 100),
    ...
    for user in users:
        for i in range(0, random.randint(1, 12)):
            ...
            period_start_on = fake.date_time_between(
                start_date='-30d',
                end_date='-20d').strftime('%Y-%m-%dT%H:%M:%SZ')
            period_end_on = fake.future_datetime(
                end_date='+14d').strftime('%Y-%m-%dT%H:%M:%SZ')
            exp_date = fake.date_time_between(
                start_date='now',
                end_date='+2y').strftime('%Y-%m-%dT%H:%M:%SZ')
